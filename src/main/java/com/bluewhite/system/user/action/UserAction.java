...
public class UserAction {
    ...
    private ClearCascadeJSON clearCascadeJSONTemporaryUser;
    {
        clearCascadeJSONTemporaryUser = ClearCascadeJSON.get().addRetainTerm(TemporaryUser.class, "userName", "id",
            "phone", "idCard", "bankCard1", "group", "status", "turnWorkTime").addRetainTerm(Group.class, "name", "id");
    }

    private ClearCascadeJSON clearCascadeJSONUserContract;
    {
        clearCascadeJSONUserContract = ClearCascadeJSON.get()
            .addRetainTerm(UserContract.class, "id", "number", "username", "archives", "pic", "idCard", "bankCard",
                "physical", "qualification", "formalSchooling", "agreement", "secrecyAgreement", "contract", "remark",
                "quit", "user")
    ...
    @ResponseBody
    @SysLogAspectAnnotation(description = "员工新增操作", module = "员工管理", operateType = "增加",
        ...
        for (Date date : dateList) {
            ...
            if (temporarily.getTemporaryUserId() != null) {
                if (temporarilyDao.findByTemporaryUserIdAndTemporarilyDateAndTypeAndGroupId(
                    temporarily.getTemporaryUserId(), date, temporarily.getType(), temporarily.getGroupId()) != null) {
    ...
    @ResponseBody
    @SysLogAspectAnnotation(description = "员工修改操作", module = "修改管理", operateType = "修改",
        logType = SysLog.ADMIN_LOG_TYPE)
