...
public class AttendanceTimeServiceImpl extends BaseServiceImpl<AttendanceTime, Long> implements AttendanceTimeService {
    ...
    public List<AttendanceTime> findAttendanceTime(AttendanceTime attendance) {
        ...
        if (!StringUtils.isEmpty(attendance.getOrgNameId())) {
            ...
            ae.setOrderTimeBegin(DateUtil.offsetHour(attendance.getOrderTimeBegin(),(int)5.5).toJdkDate());
            ae.setOrderTimeEnd(DateUtil.offsetHour(DatesUtil.getLastDayOfMonth(attendance.getOrderTimeBegin()),(int)5.5).toJdkDate());
            allAttList =
                attendanceService.findPageAttendance(ae, new PageParameter(0, Integer.MAX_VALUE)).getRows();
        }
        if (!StringUtils.isEmpty(attendance.getUserId())) {
            ...
            allAttList = attendanceDao.findByUserIdAndTimeBetween(attendance.getUserId(),
                DateUtil.offsetHour(attendance.getOrderTimeBegin(),(int)5.5).toJdkDate(),DateUtil.offsetHour(DatesUtil.getLastDayOfMonth(attendance.getOrderTimeBegin()),(int)5.5));
            userList.add(user);
        }
        if (attendance.getUserId() == null && attendance.getOrgNameId() == null) {
        ...
        Date beginTimes = DatesUtil.dayTime(attendance.getOrderTimeBegin(), "05:30:00");
        for (int i = 0; i < size; i++) {
            // 获取一天的签到结束时间,第二天的6点之前
            Date endTimes = DatesUtil.nextDay(beginTimes);
            // 满足于输入时间内离职和入职
            userList = userList.stream().filter(User -> (User.getQuit() != null && User.getQuit() == 0)
                || (User.getQuitDate() != null && User.getQuitDate().compareTo(attendance.getOrderTimeBegin()) != -1))
                .collect(Collectors.toList()).stream()
                .filter(User -> User.getEntry() != null
                    && User.getEntry().compareTo(DatesUtil.getLastDayOfMonth(attendance.getOrderTimeBegin())) != 1)
            ...
            for (User us : userList) {
                ...
                List<Attendance> attUserListSort = allAttList.stream()
                    .filter(Attendance -> Attendance.getUserId() != null && Attendance.getUserId().equals(us.getId()))
                    .collect(Collectors.toList());
                ...
                if (attendance.getUserId() == null && attendance.getOrgNameId() == null) {
                    List<AttendanceInit> attendanceInitUserList =
                        attendanceInitList.stream().filter(AttendanceInit -> AttendanceInit.getUserId() != null
                            && AttendanceInit.getUserId().equals(us.getId())).collect(Collectors.toList());
                ...
                if (attendance.getUserId() != null || attendance.getOrgNameId() != null) {
                    if (attendanceInit.getComeWork() == 2) {
                        afterAttendance = attendanceDao.findByUserIdAndTimeBetween(us.getId(),
                            DatesUtil.dayTime(beginTimes, "00:00:00"), DatesUtil.dayTime(beginTimes, "05:30:00"));
                    ...
                    if (attendanceInit.getComeWork() == 3) {
                        afterAttendance = attendanceDao.findByUserIdAndTimeBetween(us.getId(),
                            DatesUtil.dayTime(beginTimes, "00:30:00"), DatesUtil.dayTime(beginTimes, "05:30:00"));
                        if (afterAttendance.size() > 0) {
...
//                            }
    ...
    public List<Map<String, Object>> findAttendanceTimeCollectList(AttendanceTime attendanceTime) {
        ...
        // 按人员分组
        Map<Long, List<AttendanceTime>> mapAttendance =
            attendanceTimeList.stream().filter(AttendanceTime -> AttendanceTime.getUserId() != null)
        ...
        for (Long ps1 : mapAttendance.keySet()) {
            ...
            List<AttendanceTime> attendanceTimeList1 =
                psList1.stream().sorted(Comparator.comparing(AttendanceTime::getTime)).collect(Collectors.toList());
            AttendanceCollect attendanceCollect =
                attendanceCollectDao.findByUserIdAndTime(ps1, attendanceTime.getOrderTimeBegin());
    ...
    public void checkAttendanceTime(AttendanceTime attendanceTime) {
        ...
        if (!exTwo.equals("")) {
            throw new ServiceException("员工" + exTwo + "在"
                + (new SimpleDateFormat("yyyy-MM")).format(attendanceTime.getOrderTimeBegin()) + "月份的考勤已经存档，不能重新计算");
        }
    }
            ...
            for (ApplicationLeave al : applicationLeave) {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    ...
                    if (al.isHoliday() && dateArr.length >= 2) {
                        // 获取请假所有日期
                        List<Date> dateList =
                            DatesUtil.getPerDaysByStartAndEndDate(dateArr[0], dateArr[1], "yyyy-MM-dd");
                        for (Date inTime : dateList) {
                            ...
                            if (oneAtList.size() > 0) {
                                ...
                                if(al.getHolidayType() == 6) {
                                    ...
                                    oneAtList.get(0).setDutytime(oneAtList.get(0).getDutytime() != 0.0 ?oneAtList.get(0).getDutytime() : time >= turnWorkTime ? turnWorkTime : time);
                    ...
                    if (al.isApplyOvertime()) {
                        ...
                        List<AttendanceTime> applyAttendanceTime =
                            psList1.stream().filter(AttendanceTime -> (AttendanceTime.getTime().compareTo(inTime)) == 0)
                                .collect(Collectors.toList());
                        if (applyAttendanceTime.size() > 0) {
                            ...
                            at.setOvertime(NumUtils.sum(NumUtils.setzro(at.getOrdinaryOvertime()),
                                NumUtils.setzro(at.getProductionOvertime())));
                        }
                    }
    ...
    public List<Map<String, Object>> workshopAttendanceContrast(AttendanceTime attendanceTime) {
        ...
        Map<Long, List<AttendancePay>> mapAttendance =
            attendancePayList.stream().filter(AttendancePay -> AttendancePay.getUserId() != null)
