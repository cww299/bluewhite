...
public class ApplicationLeaveServiceImpl extends BaseServiceImpl<ApplicationLeave, Long>
			...
			if (applicationLeave.isAddSignIn()) {
				...
				if (addDate.length > 0) {
					for (String ad : addDate) {
						...
						attendance.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(ad));
	...
	public void defaultRetroactive(ApplicationLeave applicationLeave) throws ParseException {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		// 检查当前月份属于夏令时或冬令时 flag=ture 为夏令时
		boolean flag = DatesUtil.belongCalendar(sdf.parse(applicationLeave.getTime()));
		// 上班开始时间
		Date workTime = null;
		// 上班结束时间
		Date workTimeEnd = null;
		...
		if (flag) {
			String[] workTimeArr = attendanceInit.getWorkTimeSummer().split(" - ");
			// 将 工作间隔开始结束时间转换成当前日期的时间
			workTime = DatesUtil.dayTime(applicationLeave.getWriteTime(), workTimeArr[0]);
			workTimeEnd = DatesUtil.dayTime(applicationLeave.getWriteTime(), workTimeArr[1]);
		}else{
			// 冬令时
			String[] workTimeArr = attendanceInit.getWorkTimeWinter().split(" - ");
			// 将 工作间隔开始结束时间转换成当前日期的时间
		...
		jsonObject.put("date", applicationLeave.getSign()==0 ? workTime : workTimeEnd);
		...
		applicationLeave.setWriteTime(DatesUtil.getLastDayOfMonth(sdf.parse(applicationLeave.getTime())));
