...
public class RewardServiceImpl extends BaseServiceImpl<Reward, Long>
	...
	public PageResult<Reward> findPage(Reward reward, PageParameter page) {
		Page<Reward> pages = dao.findAll((root, query, cb) -> {
			...
			// 按日期
			if (!StringUtils.isEmpty(reward.getOrderTimeBegin()) && !StringUtils.isEmpty(reward.getOrderTimeEnd())) {
				predicate.add(cb.between(root.get("time").as(Date.class), reward.getOrderTimeBegin(),
						reward.getOrderTimeEnd()));
			}
