		...
		if (!StringUtils.isEmpty(ids)) {
			...
			if (idArr.length > 0) {
				for (int i = 0; i < idArr.length; i++) {
					...
					double sumResidueNumber = orderProcurementSet.stream().mapToDouble(OrderProcurement::getResidueNumber).sum();
					if(sumResidueNumber<ot.getDosage()){
						throw new ServiceException("当前库存不足，无法出库，请核对库存采购后进行出库");
					}
