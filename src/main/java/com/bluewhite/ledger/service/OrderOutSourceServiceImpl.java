...
public class OrderOutSourceServiceImpl extends BaseServiceImpl<OrderOutSource, Long> implements OrderOutSourceService {
        ...
        if (!StringUtils.isEmpty(orderOutSource.getOutsourceTaskIds())) {
            ...
            if (idStrings.length > 0) {
                for (String ids : idStrings) {
                    ...
                    // 查找该加工单该工序的通过审核的退货单
                    List<Integer> returnNumberList =
                        refundBillsDao.getReturnNumber(newOrderOutSource.getMaterialRequisitionId(), id);
                    // 退货总数
    ...
    public PageResult<OrderOutSource> findPages(OrderOutSource param, PageParameter page) {
        ...
        Page<OrderOutSource> pages = dao.findAll((root, query, cb) -> {
            ...
            if (param.getOutsourceTaskId() != null) {
                Join<OrderOutSource, BaseData> join =
                    root.join(root.getModel().getSet("outsourceTask", BaseData.class), JoinType.LEFT);
            ...
            if (!StringUtils.isEmpty(param.getOutSourceNumber())) {
                predicate
                    .add(cb.like(root.get("outSourceNumber").as(String.class), "%" + param.getOutSourceNumber() + "%"));
            }
            // 是否审核
        ...
        result.getRows().forEach(o -> {
            // 获取加工退货单后的实际数量
            List<RefundBills> listRefundBills = refundBillsDao.findByOrderOutSourceId(o.getId());
            o.setActualQuantity(o.getProcessNumber());
            if (listRefundBills.size() > 0) {
                int refundBillsNumber = listRefundBills.stream().mapToInt(RefundBills::getReturnNumber).sum();
                o.setRefundBillsNumber(refundBillsNumber);
                o.setActualQuantity(o.getProcessNumber()-refundBillsNumber);
            }
            if (warehouseTypeDeliveryId != null) {
                ...
                if (outStorageList.size() > 0) {
                    int sendNumber =
                        outStorageList.stream().filter(OutStorage -> OutStorage.getOrderOutSourceId().equals(o.getId()))
                            .mapToInt(OutStorage::getArrivalNumber).sum();
                    // 皮壳剩余数量
    ...
    public List<Map<String, Object>> mixOutSoureRefund(Long id) {
        ...
        outsourceTask.stream().forEach(outB -> {
            Map<String, Object> map = new HashMap<>();
            List<ProcessPrice> pList =
                processPriceList.stream().filter(ProcessPrice -> ProcessPrice.getProcessTaskId().equals(outB.getId()))
