...
import com.bluewhite.common.annotation.ClearCascade;
import com.bluewhite.common.annotation.Extract;
import com.bluewhite.common.annotation.ExtractRenameTerm;
import com.bluewhite.common.annotation.FilterRetainTerm;
 ...
 * 本方法还支持将DATE格式转化成Timestamp。默认转化格式为yyyy-mm-dd
 * HH:mm:ss，如需打开{@link #dateToTimestamp()}}
 *
 */
public class ClearCascadeJSON {
	...
	/**
	 * 萃取一个属性，将级联的某个属性作为上级的属性,暂时仅支持二级属性a.b不支持3级a.b.c <b>该方法仅对输出格式为JSON有效</b>
	 * 
	 * @param finalAtt
	 *            目标参数名
	 * @param sourceAttRef
	...
	public String toJSONString() {
		...
		if (javaObject instanceof Collection) {
			return toJSONString(javaObject, myJsonFilter, SerializerFeature.WriteNullStringAsEmpty,
	...
	public final String toJSONString(Object object, SerializeFilter filter, SerializerFeature... features) {
		if (useDateFormat) {
			return JSON.toJSONString(object, SerializeConfig.globalInstance, new SerializeFilter[] { filter },
					JSON.DEFFAULT_DATE_FORMAT,JSON.DEFAULT_GENERATE_FEATURE, features);
		}
...
class MyJsonFilter implements PropertyPreFilter {
	...
	public boolean apply(JSONSerializer serializer, Object source, String name) {
		...
		long sourceClassFilterCount = includes.entrySet().parallelStream()
				.filter(i -> i.getKey().isAssignableFrom(clazz)).count();
