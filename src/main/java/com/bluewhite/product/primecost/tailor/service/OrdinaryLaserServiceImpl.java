...
public class OrdinaryLaserServiceImpl extends BaseServiceImpl<OrdinaryLaser, Long> implements OrdinaryLaserService {
	...
	public OrdinaryLaser saveOrdinaryLaser(OrdinaryLaser ordinaryLaser) {
		...
		case 73:// 手工电烫
			...
			// 电烫秒数（含快手)
			ordinaryLaser.setPermSeconds(primeCoefficient.getPermThree() / ordinaryLaser.getTypesettingNumber()
			...
			// 拉布秒数（含快手)
			ordinaryLaser.setRabbTime(primeCoefficient.getPermOne() / (1.5 / ordinaryLaser.getTailorSize().getOrdinaryLaser())
					* primeCoefficient.getQuickWorker());
			// 电烫工价（含快手)
			ordinaryLaser.setPermPrice((ordinaryLaser.getPermSeconds() + ordinaryLaser.getRabbTime())
			...
			ordinaryLaser
					.setEquipmentPrice((primeCoefficient.getDepreciation() + primeCoefficient.getLaserTubePriceSecond()
							+ primeCoefficient.getMaintenanceChargeSecond() + primeCoefficient.getPerSecondPrice())
							* (ordinaryLaser.getPermSeconds() + ordinaryLaser.getRabbTime()));
			// 管理人员费用
			ordinaryLaser.setAdministrativeAtaff(primeCoefficient.getPerSecondManage() * (ordinaryLaser.getPermSeconds()
					+ ordinaryLaser.getTearingSeconds() + ordinaryLaser.getRabbTime()));
			break;
		case 74:// 设备电烫

			break;
		case 75:// 冲床
			managePrice = materielService.getBaseThreeOne(ordinaryLaser.getTailorTypeId(),ordinaryLaser.getTailorSizeId());
			// 得到理论(市场反馈）含管理价值
			ordinaryLaser.setManagePrice(managePrice);
			// 叠布秒数（含快手)
			ordinaryLaser.setOverlappedSeconds(
					(ordinaryLaser.getNumber() < primeCoefficient.getPuncherTwo() ? primeCoefficient.getPuncherFour()
							...
							/ ((ordinaryLaser.getLayerNumber() * primeCoefficient.getPuncherThree() * 1.5)
									/ (ordinaryLaser.getTailorSize().getOrdinaryLaser() * primeCoefficient.getQuickWorker()))

			);
			ordinaryLaser.setPunchingSeconds(primeCoefficient.getPuncherFive() / ordinaryLaser.getLayerNumber()
					* primeCoefficient.getQuickWorker());

			// 工价（含快手)
			ordinaryLaser.setLabourCost((ordinaryLaser.getOverlappedSeconds() + ordinaryLaser.getPunchingSeconds()
			...
			ordinaryLaser
					.setEquipmentPrice((primeCoefficient.getDepreciation() + primeCoefficient.getLaserTubePriceSecond()
							+ primeCoefficient.getMaintenanceChargeSecond() + primeCoefficient.getPerSecondPrice())
							* (ordinaryLaser.getOverlappedSeconds() + ordinaryLaser.getPunchingSeconds()
									+ ordinaryLaser.getOtherTimeOne() + ordinaryLaser.getOtherTimeThree()
									+ ordinaryLaser.getOtherTimeTwo()));

			// 管理人员费用
			ordinaryLaser
					.setAdministrativeAtaff((ordinaryLaser.getOverlappedSeconds() + ordinaryLaser.getPunchingSeconds())
		...
		case 76:// 电推
			...
			// 电推秒数（含快手)
			ordinaryLaser.setElectricSeconds((ordinaryLaser.getPerimeter() / primeCoefficient.getElectricPushFive()
					* primeCoefficient.getElectricPushSix()) / ordinaryLaser.getLayerNumber());
			// 工价（含快手)
			ordinaryLaser.setLabourCost((ordinaryLaser.getOverlappedSeconds() + ordinaryLaser.getElectricSeconds())
			...
			// 管理人员费用
			ordinaryLaser
					.setAdministrativeAtaff((ordinaryLaser.getOverlappedSeconds() + ordinaryLaser.getElectricSeconds())
							* ordinaryLaser.getSingleLaserHandTime());
			break;
		case 77:// 手工剪刀
			managePrice = materielService.getBaseThreeOne(ordinaryLaser.getTailorTypeId(),
		...
		// 普通激光切割该裁片费用
		ordinaryLaser.setStallPrice((ordinaryLaser.getLabourCost() + ordinaryLaser.getEquipmentPrice()
