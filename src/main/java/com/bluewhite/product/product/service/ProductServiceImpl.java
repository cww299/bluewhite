...
public class ProductServiceImpl extends BaseServiceImpl<Product, Long> implements ProductService {
	...
	public PageResult<Product> findPages(Product param, PageParameter page) {
		...
		if (cu.getRole().contains(Constants.TRIALPRODUCT)) {
			param.setOriginDepartment(Constants.TRIALPRODUCT);
		}
		// 质检
		...
		if (cu.getRole().contains(Constants.PRODUCT_TWO_MACHINIST)) {
			param.setOriginDepartment(Constants.PRODUCT_TWO_MACHINIST);
		}
		// 裁剪
	...
	public PrimeCost getPrimeCost(PrimeCost primeCost) {
		...
		double batchMaterialPrice = cutPartsList.stream().filter(CutParts -> CutParts.getBatchMaterialPrice() != null)
				.mapToDouble(CutParts::getBatchMaterialPrice).sum();
		double batchComplexMaterialPrice = cutPartsList.stream()
				.filter(CutParts -> CutParts.getBatchComplexMaterialPrice() != null)
				.mapToDouble(CutParts::getBatchComplexMaterialPrice).sum();
		double batchComplexAddPrice = cutPartsList.stream()
				.filter(CutParts -> CutParts.getBatchComplexAddPrice() != null)
				.mapToDouble(CutParts::getBatchComplexAddPrice).sum();
		...
		double cutPartsPriceInvoice = primeCost.getCutPartsInvoice() == 1
				? (primeCost.getCutPartsPriceInvoice() * primeCost.getOneCutPartsPrice()) : 0;

		// 除面料以外的其他物料价格
		List<ProductMaterials> productMaterialsList = productMaterialsDao.findByProductId(primeCost.getProductId());
		double batchMaterialPrice1 = productMaterialsList.stream()
				.filter(ProductMaterials -> ProductMaterials.getBatchMaterialPrice() != null)
				.mapToDouble(ProductMaterials::getBatchMaterialPrice).sum();
		...
		double otherCutPartsPriceInvoice = primeCost.getOtherCutPartsPriceInvoice() == 1
				? (primeCost.getOtherCutPartsPriceInvoice() * primeCost.getOneOtherCutPartsPrice()) : 0;

		// 裁剪
		List<Tailor> tailorList = tailorDao.findByProductId(primeCost.getProductId());
		double allCostPriceTailor = tailorList.stream().filter(Tailor -> Tailor.getAllCostPrice() != null)
				.mapToDouble(Tailor::getAllCostPrice).sum();
		...
		double cutPriceInvoice = primeCost.getCutPriceInvoice() == 1
				? (primeCost.getCutPriceInvoice() * primeCost.getOneCutPrice()) : 0;

		// 机工
		List<Machinist> machinistList = machinistDao.findByProductId(primeCost.getProductId());
		double allCostPriceMachinist = machinistList.stream().filter(Machinist -> Machinist.getAllCostPrice() != null)
		...
		double machinistPriceInvoice = primeCost.getMachinistPriceInvoice() == 1
				? (primeCost.getMachinistPriceInvoice() * primeCost.getOneMachinistPrice()) : 0;

		// 绣花
		List<Embroidery> embroideryList = embroideryDao.findByProductId(primeCost.getProductId());
		double allCostPriceEmbroidery = embroideryList.stream()
				.filter(Embroidery -> Embroidery.getAllCostPrice() != null).mapToDouble(Embroidery::getAllCostPrice)
				.sum();
		...
		double embroiderPriceInvoice = primeCost.getEmbroiderPriceInvoice() == 1
				? (primeCost.getEmbroiderPriceInvoice() * primeCost.getOneEmbroiderPrice()) : 0;

		// 针工
		List<Needlework> needleworkList = needleworkDao.findByProductId(primeCost.getProductId());
		double allCostPriceNeedlework = needleworkList.stream()
				.filter(Needlework -> Needlework.getAllCostPrice() != null).mapToDouble(Needlework::getAllCostPrice)
				.sum();
		...
		double needleworkPriceInvoice = primeCost.getNeedleworkPriceInvoice() == 1
				? (primeCost.getNeedleworkPriceInvoice() * primeCost.getOneNeedleworkPrice()) : 0;

		// 包装
		List<Pack> packList = packDao.findByProductId(primeCost.getProductId());
		double allCostPricePack = packList.stream().filter(Pack -> Pack.getAllCostPrice() != null)
				.mapToDouble(Pack::getAllCostPrice).sum();
		primeCost.setPackPrice(allCostPricePack);
		primeCost.setOnePackPrice(NumUtils.division((allCostPricePack) / primeCost.getNumber()));
		double packPriceInvoice = primeCost.getPackPriceInvoice() == 1
				? (primeCost.getPackPriceInvoice() * primeCost.getOnePackPrice()) : 0;
		...
		// 实战单只成本
		primeCost.setOneaAtualPrimeCost(primeCost.getCutPartsPricePricing() + primeCost.getCutPricePricing()
				+ primeCost.getEmbroiderPricePricing() + primeCost.getMachinistPricePricing()
				+ primeCost.getEmbroiderPricePricing() + primeCost.getPackPricePricing()
				+ primeCost.getFreightPricePricing());
		// 预计运费价
		primeCost.setFreightPrice(primeCost.getAdjustNumber() * primeCost.getOneFreightPrice());
		// 付上游开票点()
		primeCost.setUpstream(cutPartsPriceInvoice + otherCutPartsPriceInvoice + cutPriceInvoice + machinistPriceInvoice
				+ embroiderPriceInvoice + needleworkPriceInvoice + packPriceInvoice);
		// 付国家的
		primeCost.setState(NumUtils.division(primeCost.getInvoice() == 1 ? primeCost.getState() / 1.17 * 0.17
				- (primeCost.getOneCutPartsPrice() + primeCost.getOneOtherCutPartsPrice() + primeCost.getOneCutPrice()
						+ primeCost.getOneMachinistPrice() + primeCost.getOneEmbroiderPrice()
						+ primeCost.getOnePackPrice() + primeCost.getOneNeedleworkPrice() + primeCost.getOnePrimeCost())
				: 0.0));
		// 预计多付国家的
		primeCost.setExpectState(primeCost.getInvoice() == 1
				? primeCost.getState() - (primeCost.getSale() * primeCost.getTaxes()) : 0.0);
		// 考虑多付国家的不付需要的进项票点
		primeCost.setNoState(
				NumUtils.division(primeCost.getInvoice() == 1 ? primeCost.getState() / 0.17 * 1.17 * 0.08 : 0.0));
		// 付返点和版权点
		primeCost.setRecidivate(
				(primeCost.getSale() * primeCost.getRebateRate()) + (primeCost.getSale() * primeCost.getRebateRate()));
		// 付运费
		primeCost.setFreight(primeCost.getFreightPrice());
		// 剩余到手的
		primeCost.setSurplus(primeCost.getSale() - primeCost.getState() - primeCost.getUpstream()
				- primeCost.getRecidivate() - primeCost.getFreight() - primeCost.getNoState());
		// 实际加价率
		primeCost.setMakeRate(NumUtils.division(primeCost.getSurplus() / primeCost.getOnePrimeCost()));
		// 目前综合税负加所得税负比
		primeCost.setTaxesRate(
				NumUtils.division((primeCost.getState() - primeCost.getExpectState()) / (primeCost.getSale() / 1.17)));
		// 预算成本
