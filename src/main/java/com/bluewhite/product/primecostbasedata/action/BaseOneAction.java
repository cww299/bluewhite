...
public class BaseOneAction {
    ...
    @ResponseBody
    public CommonResponse getBaseOne(BaseOne baseOne) {
        CommonResponse cr = new CommonResponse();
        cr.setData(
            ClearCascadeJSON.get().addRetainTerm(BaseOne.class, "id", "name", "textualTime", "time", "baseOneTimes")
    ...
    public CommonResponse getMaterielPage(Materiel materiel, PageParameter page) {
        ...
        cr.setData(ClearCascadeJSON.get()
            ...
            .addRetainTerm(OrderProcurement.class, "id", "orderProcurementNumber", "placeOrderNumber", "arrivalNumber",
                ...
                "squareGram")
            .addRetainTerm(BaseData.class, "id", "name").format(materielService.findMaterielPages(materiel, page))
            .toJSON());
        cr.setMessage("成功");
        return cr;
    }
    ...
    @ResponseBody
    @SysLogAspectAnnotation(description = "基础数据新增操作", module = "新增管理", operateType = "新增",
        logType = SysLog.ADMIN_LOG_TYPE)
    public CommonResponse addMateriel(Materiel materiel) {
        CommonResponse cr = new CommonResponse();
    ...
    @ResponseBody
    public CommonResponse updatePrimeCoefficient(PrimeCoefficient primeCoefficient) {
        CommonResponse cr = new CommonResponse();
        ...
        primeCoefficient.setDepreciation(primeCoefficient.getWorth() / primeCoefficient.getShareDay()
            / primeCoefficient.getWorkTime()
            / (primeCoefficient.getNeedleworkOne() != null ? primeCoefficient.getNeedleworkOne() : 1.0) / TIME / TIME);
        // 每秒激光管费用
        primeCoefficient.setLaserTubePriceSecond(primeCoefficient.getLaserTubePrice() / primeCoefficient.getShareTime()
            / (primeCoefficient.getNeedleworkOne() != null ? primeCoefficient.getNeedleworkOne() : 1.0) / TIME / TIME);
        // 每秒维护费用
        primeCoefficient.setMaintenanceChargeSecond(primeCoefficient.getMaintenanceCharge()
            / primeCoefficient.getShareTimeTwo()
            / (primeCoefficient.getNeedleworkOne() != null ? primeCoefficient.getNeedleworkOne() : 1.0) / TIME / TIME);
        // 每秒耗3费
        primeCoefficient.setPerSecondPrice((primeCoefficient.getOmnHorElectric() + primeCoefficient.getOmnHorWater()
            + primeCoefficient.getOmnHorHouse())
            / (primeCoefficient.getNeedleworkOne() != null ? primeCoefficient.getNeedleworkOne() : 1.0) / TIME / TIME);
        // 每秒工价
        ...
        if (primeCoefficient.getNeedleworkOne() != null) {
            // 每秒耗3费
            primeCoefficient.setPerSecondPriceTwo((primeCoefficient.getNeedleworkTwo()
                + primeCoefficient.getNeedleworkThree() + primeCoefficient.getNeedleworkFour())
                / primeCoefficient.getManageEquipmentNumber() / TIME / TIME);
        ...
        // 每秒管理费用
        primeCoefficient.setPerSecondManage(
            primeCoefficient.getManagePrice() / primeCoefficient.getManageEquipmentNumber() / 25 / 8 / TIME / TIME);
    ...
    @ResponseBody
    public CommonResponse getPrimeCoefficient(Long id) {
        CommonResponse cr = new CommonResponse();
    ...
    @ResponseBody
    public CommonResponse getBaseFour(String sewingOrder) {
        CommonResponse cr = new CommonResponse();
    ...
    @ResponseBody
    @SysLogAspectAnnotation(description = "物料价格变动修改操作", module = "产品价格表", operateType = "修改",
        logType = SysLog.ADMIN_LOG_TYPE)
    public CommonResponse updateMaterielAndOther(Long productId) {
        CommonResponse cr = new CommonResponse();
        // 裁片
        List<CutParts> cutPartsList = cutPartsService.findByProductId(productId);
        cutPartsList =
            cutPartsList.stream().filter(CutParts -> CutParts.getMaterielId() != null).collect(Collectors.toList());
