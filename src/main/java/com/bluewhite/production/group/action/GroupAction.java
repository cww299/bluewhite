...
public class GroupAction {
    ...
    /**
     * 添加，修改分组 type 分组所属部门类型 (1=一楼质检，2=一楼包装，3=二楼针工,4=二楼机工)
     * 
     * @return cr
     */
    ...
    public CommonResponse allGroup(HttpServletRequest request, Long id, Date temporarilyDate) {
        ...
        List<Temporarily> temporarilyList =
            temporarilyDao.findByTypeAndGroupIdAndTemporarilyDateBetween(group.getType(), id, startTime, endTime);
        ...
        if (UnUtil.isFromMobile(request)) {
            ...
            List<AttendancePay> attendancePayList =
                attendancePayDao.findByGroupIdAndTypeAndAllotTimeBetween(id, group.getType(), startTime, endTime);
            if (temporarilyList.size() > 0) {
                for (Temporarily temporarily : temporarilyList) {
                    // 查询出该分组临时员工的出勤数据
                    if (temporarily.getTemporaryUserId() != null) {
    ...
    public CommonResponse getTemporarilyList(Temporarily temporarily) {
        ...
        cr.setData(ClearCascadeJSON.get()
            ...
            .addRetainTerm(User.class, "userName").addRetainTerm(Group.class, "name").format(temporarilyList).toJSON());
        cr.setMessage("查询成功");
        return cr;
    }
        ...
        if (!StringUtils.isEmpty(ids)) {
            ...
            for (String id : idString) {
                ...
                if (temporarily != null) {
                    ...
                    List<FarragoTask> farragoTaskList =
                        farragoTaskService.findInSetIds(id, orderTimeBegin, orderTimeEnd);
                    if (taskList.size() > 0 || farragoTaskList.size() > 0) {
                        cr.setCode(ErrorCode.ILLEGAL_ARGUMENT.getCode());
                        cr.setMessage((temporarily.getTemporaryUser() == null ? temporarily.getUser().getUserName()
                            : temporarily.getTemporaryUser().getUserName()) + "当天考勤已分配任务，无法删除，需删除，请先删除任务");
                        return cr;
                    } else {
