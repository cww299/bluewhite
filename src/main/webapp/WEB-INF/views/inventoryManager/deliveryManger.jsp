...
layui.config({
	...
	function(){
		...
		table.render({
			elem:'#packTable',
			url:'${ctx}/ledger/packingPage?type=1',
			where: { flag:0 },
			toolbar:'#packToolbar',
			page:true,
			size:'lg',
		...
		$('#sureAddEidtBtn').on('click',function(obj){
			...
			if(addEditId!=''){
				layui.each(table.cache['childTable'],function(index,item){
					if(item.id && item.id!=''){
						child.push({
							lastPackingChildId: item.lastPackingChildId,
							count: item.count,
			...
			var msg = '';
			customerId=='' && (msg='发货单客户不能为空');
			packingDate=='' && (msg='发货单发货日期不能为空');
		...
		renderTable({
			...
			cols: [[
					{align:'center',type:'checkbox'},
			        {align:'center',field:'lastPackingChildId',title:'日期 ~ 批次号 ~ 产品 ~ 可发货数量',edit:false, templet: getSelectHtml('childTable','bacthNumber')},
			...
			return function(d){
				...
				if(tid=="childTable" && d.id){	//如果是子单且是修改的数据
					for(var i=0;i<data.length;i++){
						if(data[i].id == d.lastPackingChildId){
		...
		form.on('select(tableSelect)',function(obj){
			...
			var tableId = $(obj.elem).data('table');
		...
		function addTempData(tid,data){
			...
			if(tid=='childTable'){
				allField = {  lastPackingChildId:'', count:0 };
			}
		...
		function addEdit(addEditType){
			...
			var childData = [],materialData = [],cusId = '',userId='', warehouseTypeId='';
			addEditId = '';
			$('#addEditCustomer').val('');
		...
		function getAllSend(surplusNumber){
			...
			allSend = myutil.getDataSync({
				url:'${ctx}/ledger/packingChildList?confirm=1&flag=1'+field,	
			});
		}
