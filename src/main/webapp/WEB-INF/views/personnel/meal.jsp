			...
			layui.config({
				...
				function() {
					...
					table.on('toolbar(tableData)', function(obj) {
						...
						switch(obj.event) {
							case 'addTempData':
								allField = {id: '', mode: '',userId:'',tradeDaysTime:'',price:''};
								table.addTemp(tableId,allField,function(trElem) {
									console.log(trElem)
									// 进入回调的时候this是当前的表格的config
									var that = this;
									// 初始化laydate
									layui.each(trElem.find('td[data-field="tradeDaysTime"]'), function(index, tdElem) {
										tdElem.onclick = function(event) {
											layui.stope(event)
										};
										laydate.render({
											...
											format: 'yyyy-MM-dd HH:mm:ss',
												...
												var tableView = trElem.closest('.layui-table-view');
												table.cache[that.id][trElem.data('index')]['tradeDaysTime'] = value;
												var id = table.cache[tableView.attr('lay-id')][trElem.data('index')].id
											}
										})
									})
