...
(function ($) {
    ...
    var checkboxTool = {
        ...
        renderData: function (R, $grid, cyProps) {
            ...
            //获取下拉控件的默认值
            var _value = $(_grid).attr("data_value");
            ...
            if (data !== undefined) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (_values.indexOf(data[i].id+'') == -1) {
                        var _input = '<dd lay-value="' + data[i].id + '"  lay-name="' + _name + '"  lay-title="' + data[i].userName + '"><input type="checkbox" lay-name="' + _name + '"  lay-filter="transferLeftChecked" title="' + data[i].userName + '" lay-skin="primary"></dd>';
                        //设置禁用
                    ...
                    //像左侧插入元素
                        ...
                        var _input = '<dd lay-value="' + data[i].id + '" lay-name="' + _name + '"  lay-title="' + data[i].userName + '"><input type="hidden" name="' + _name + '" value="' + data[i].id + '"><input lay-filter="transferRightChecked"   type="checkbox"  title="' + data[i].userName + '" lay-skin="primary"></dd>';
                        //设置禁用
                        if (_disableds.indexOf(data[i].id) != -1) {
                            _input = _input.replace("<input", "<input disabled ")
                        }
            ...
            var outHtml =
                _grid.html([
                    ...
                    '<button title="右移" type="button" lay-name="' + _name + '" class="layui-btn layui-btn-normal layui-btn-sm layui-btn-disabled"><i class="layui-icon">&#xe65b;</i></button>',
                    '</div>',
                    '<div class="transfer-btn  transfer-to-left">',
                    '<button title="左移" type="button" lay-name="' + _name + '"  class="layui-btn layui-btn-normal layui-btn-sm layui-btn-disabled"><i class="layui-icon">&#xe65a;</i></button>',
                    '</div>',
                    ...
                    '</span>',
                    '</dd>',
                    '<div id="fightDiv" class="transfer-div">',
                    rightList,
                    '</div>',
                    '</div>',
                    '</div>'
    ...
    $(document).on("click", ".transfer-to-right", function () {
        var $this = $(this);
         /*var _name =  "userId"  $this.attr("lay-name") || "";*/
        ...
        for (var i = 0; i < inputs.length; i++) {
            if ($(inputs[i]).is(':checked')) {
                ...
                var _name = $(inputs[i]).parents("dd").attr("lay-name");
