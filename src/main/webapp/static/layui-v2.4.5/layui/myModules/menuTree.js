	 ...
	 /*     name:'我的一个道姑朋友',
		  ...
		  createTree:function(){
			  ...
			  if(this.data.length==0)
				  ...
				  this.result = '<div class="layui-form">'+tree(this.data,'',this.checked)+'</div>';
				  function tree(data,nbsp,checked){
					  var html='<div>';
					  for(var i=0; i < data.length; i++){
						  ...
						  html+='<input type="checkbox" name="treeCheckbox" value="'+data[i].id+'" '+c+' lay-skin="primary">';		//复选框
						  html+='<i class="layui-icon layui-icon-'+data[i].icon+'"></i>&nbsp;&nbsp;'			//菜单图标
						  html+='<span>'+data[i].name+'</span>';											//菜单名
						  html+='</div>'
						  if(data[i].children!=null && data[i].children.length>0)
								 html+='<div class="layui-tree-child" style="display:none">'+tree(data[i].children,nbsp+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',checked)+'</div>';	//子菜单
						  html+='</div>'
					  }
					  return html+"</div>";
				  }
		  ...
		  showResult:function(){
			  ...
			  $('.layui-icon-up').on('click',function(obj){
				  hide(obj);
			  })
			   $('.layui-icon-down').on('click',function(obj){
				  show(obj);
			  })
			  function show(obj){
