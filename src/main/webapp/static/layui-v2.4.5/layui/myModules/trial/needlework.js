/* author:299
 * 2019/8/29   试制模块：绣花
 * needlework.render({ elem:'给定的元素。填充真正的内容', btn:'绑定的按钮' })
 */
layui.define(['mytable','element'],function(exports){
	var $ = layui.jquery,
		...
		myutil = layui.myutil;
	var html = [
	            '<div class="layui-tab layui-tab-brief" lay-filter="tabFilterNeedlework">',
					'<ul class="layui-tab-title">',
						'<li class="layui-this" style="width: 45%;" lay-id="tabFilterNeedleworkFirst">针工页面</li>',
						'<li style="width: 45%;">针工时间设定</li>',
					'</ul>',
					'<div class="layui-tab-content">',
						'<div class="layui-tab-item layui-show">',
							...
							'</tr><table>',
							'<table id="needleworkPageTable" lay-filter="needleworkPageTable"></table>',
						'</div>',
						'<div class="layui-tab-item">',
							'<table class="layui-form"  style="margin-top:8px;"><tr>',
								...
								'<td><button lay-submit type="button" lay-filter="" class="layui-btn layui-btn-sm">搜索</button></td>',
							'</tr><table>',
							'<table id="needleworkTimeTable" lay-filter="needleworkTimeTable"></table>',
						'</div>',
					'</div>',
				'</div>',
	            ].join(' ');
	...
	needlework.render = function(opt){
		var elem = opt.elem,
			btn = opt.btn;
		$('#'+elem).html(html);	//填充真正的html内容
		element.render();
		var tableId = 'needleworkPageTable';
		var tableTimeId = 'needleworkTimeTable';
		mytable.render({			//裁片表格
			elem:'#'+tableId,
			data:[],
			size:'lg',
